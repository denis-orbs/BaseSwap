...
import { Trade, TokenAmount, CurrencyAmount, ETHER, Currency, Token } from '@magikswap/sdk'

import { useCallback, useMemo } from 'react'
import { logError } from 'utils/sentry'
import { useWeb3React } from '@web3-react/core'
import { useTranslation } from '@pancakeswap/localization'
import { ROUTER_ADDRESS, ROUTER_ADDRESS_ODOS } from 'config/constants/exchange'
...
// wraps useApproveCallback in the context of a swap
// since just using in, can use just in the IN Amount
export function useApproveCallbackFromTradeV3(inputCurrency: Token, typedValue?: string, allowedSlippage = 0, chainId?: number) {
  const amountToApprove = useMemo(
    () => typedValue || undefined,
    [typedValue, allowedSlippage],
  )

  return useApproveCallbackOdos(inputCurrency, amountToApprove, ROUTER_ADDRESS_ODOS[chainId])
}


// returns a variable indicating the state of the approval and a function which approves if necessary or early returns
export function useApproveCallbackOdos(
  inputCurrency: Token,
  ...
  const approve = useCallback(async (): Promise<void> => {
    ...
    return callWithGasPrice(
      ...
      .then((response: TransactionResponse) => {
        addTransaction(response, {
          summary: `Approve ${inputCurrency.name}`,
          approval: { tokenAddress: token.address, spender },
          type: 'approve',
        })
      })

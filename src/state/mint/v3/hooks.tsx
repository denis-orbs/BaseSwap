...
export function useV3DerivedMintInfo(
  ...
  // lower and upper limits in the tick space for `feeAmoun<Text>
  ...
  if (!account) {
    errorMessage = <Text>Connect Wallet</Text>
  }

  if (poolState === PoolState.INVALID) {
    errorMessage = errorMessage ?? <Text>Invalid pair</Text>
  }

  if (invalidPrice) {
    errorMessage = errorMessage ?? <Text>Invalid price input</Text>
  }
  ...
  if (
    ...
    errorMessage = errorMessage ?? <Text>Enter an amount</Text>
  }

  const { [Field.CURRENCY_A]: currencyAAmount, [Field.CURRENCY_B]: currencyBAmount } = parsedAmounts

  if (currencyAAmount && currencyBalances?.[Field.CURRENCY_A]?.lessThan(currencyAAmount)) {
    errorMessage = <Text>Insufficient {currencies[Field.CURRENCY_A]?.symbol} balance</Text>
  }

  if (currencyBAmount && currencyBalances?.[Field.CURRENCY_B]?.lessThan(currencyBAmount)) {
    errorMessage = <Text>Insufficient {currencies[Field.CURRENCY_B]?.symbol} balance</Text>
  }
